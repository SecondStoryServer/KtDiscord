...
internal object RateLimiter {
    ...
    internal suspend fun getMutex(endPoint: EndPoint) =
        mutex.withLock { mutexes.getOrPut(endPoint.hashCode()) { Mutex() } }
        ...
        rateLimitRemaining[endPoint.hashCode()]?.let {
            ...
            if (it <= 0) {
                ...
                while (globalRateLimitEnds.coerceAtLeast(rateLimitEnds[endPoint.hashCode()]!!) > System.currentTimeMillis()) {
                    val delay =
                        globalRateLimitEnds.coerceAtLeast(rateLimitEnds[endPoint.hashCode()]!!) - System.currentTimeMillis()
